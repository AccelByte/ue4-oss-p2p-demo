...
FSocketAccelByte::FSocketAccelByte(bool IsServer):bIsServer(IsServer)
{
}
...
{
	if (bIsServer) {
		AccelByteNetworkManager::Instance().CloseAllPeer();
	}
	else {
		AccelByteNetworkManager::Instance().ClosePeerConnection(NetId.ToString());
	}
	return true;
}
...
bool FSocketAccelByte::SendTo(const uint8* Data, int32 Count, int32& BytesSent, const FInternetAddr& Destination)
{
	...
	NetId = Dest.NetId;
	return AccelByteNetworkManager::Instance().SendTo(Data, Count, BytesSent, Dest.NetId.ToString());
}
...
bool FSocketAccelByte::RecvFrom(uint8* Data, int32 BufferSize, int32& BytesRead, FInternetAddr& Source, ESocketReceiveFlags::Type Flags)
{
	...
	{
		static_cast<FInternetAddrAccelByte&>(Source).NetId.UniqueNetIdStr = DestString;
		return true;
	}
	return false;
}
...
{
	//always success becuase directly read data from webrtc data channel
	return true;
}
